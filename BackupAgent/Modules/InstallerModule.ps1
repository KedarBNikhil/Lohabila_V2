...
function Copy-ApplicationFiles {
    ...
    <#
        .SYNOPSIS
            Copies Backup Agent application files to installation directory.
    ...
    try {
       ...
       $Source = $script:InstallerConfig.PackageSourceRoot  # folder where installer is running from
        

       if (-not (Test-Path $Source)) {
    Write-Host "ERROR: Package source not found: $Source"
    return $false
}
        $Destination = $script:InstallerConfig.InstallRoot

        # Define what to copy
        $ItemsToCopy = $script:InstallerConfig.PackageItems
        ...
        foreach ($Item in $ItemsToCopy) {
            ...
            if (-not (Test-Path $SourcePath)) {
                Write-Host "WARNING: Missing source item: $Item"
                continue
            }

            # If folder → copy recursively
